...
const Typewriter = ({ text, style }) => {
  ...
  React.useEffect(() => {
    ...
    const timer = setInterval(() => {
      // FIX: Use slice instead of appending characters to prevent jumbled text
...
const ChatMessageList = ({ messages, loading = false, contentContainerStyle }) => {
  // 1. Prepare Data: Reverse messages for Inverted List
  const reversedMessages = [...messages].reverse();

  const renderItem = ({ item, index }) => {
    const isUser = item.sender === 'user';
    const isTyping = Boolean(item?.isTyping);
    // 2. Logic Update: In an inverted list, "Latest" is at index 0
      ...
      <View style={[
        ...
        <View style={[
          ...
          {!isUser && (
            <View style={styles.assistantHeader}>
              <View style={styles.botIconPlaceholder}>
                 <Text style={styles.botIconText}>N</Text>
              </View>
          ...
          {!isUser && isTyping ? (
            ...
            <Typewriter text={item.text} style={styles.assistantText} />
          ) : (
...
const styles = StyleSheet.create({
  ...
  bubble: {
    maxWidth: '85%', // Slightly wider for header
    padding: 14,
    borderRadius: 14,
  },
  userBubble: {

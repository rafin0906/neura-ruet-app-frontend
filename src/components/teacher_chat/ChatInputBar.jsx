...
const tools = [
    { id: 'generate_marksheet', label: 'Generate Marks Sheet', icon: require('../../../assets/icons/generate_cover_page_icon.png') },
...
const ChatInputBar = ({ onSend, currentTool, setCurrentTool }) => {
    ...
    const [showToolError, setShowToolError] = useState(false); // Validation State
    
    // 1. State to control bottom padding (Preserved from Teacher Logic)
    const [bottomPadding, setBottomPadding] = useState(95);

    // 2. Listen for Keyboard Events (Preserved from Teacher Logic)
    useEffect(() => {
        ...
        >
                ...
                <Modal transparent animationType="fade" visible={menuVisible} onRequestClose={() => setMenuVisible(false)}>
                    <TouchableWithoutFeedback onPress={() => setMenuVisible(false)}>
                        <View style={styles.modalOverlay}>
                            <View style={[styles.menuContainer, { bottom: bottomPadding + 80 }]}>
                                ...
                                {tools.map((tool) => (
                                    <TouchableOpacity key={tool.id} style={styles.menuItem} onPress={() => selectTool(tool)}>
            ...
            <View style={styles.inputRow}>
                <TouchableOpacity
                    ...
                    activeOpacity={0.8}
                >
                    <Image source={require('../../../assets/icons/tool_select_plus_icon.png')} style={styles.plusIcon} />
                </TouchableOpacity>
                ...
                <View style={styles.inputWrapper}>
                    ...
                    <TouchableOpacity 
                        ...
                        onPress={handleSend} 
                        activeOpacity={0.8}
                        disabled={isInputEmpty} // Disable interaction
                    >
                        <Image
                            source={require('../../../assets/icons/message_send_icon.png')}
                            style={[styles.sendIcon, isInputEmpty && styles.sendIconDisabled]}
                            resizeMode="contain"
                        />
                    </TouchableOpacity>
                </View>
